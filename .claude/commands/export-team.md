...
> 이 export는 **Claude 하네스 단독 패키지**다 (`.claude/` + `CLAUDE.md` + 공유 `.workflow/state/README.md`). Codex 하네스(`AGENTS.md`·`.codex/`)는 포함하지 않는다 — 공존 베이스 전체를 옮기려면 Codex 측 `/export-team`도 따로 실행한다.
...
- `.workflow/state/current.json` (**`{}` 빈 객체로 초기화해 포함**, 4단계 참조)
...
- `.workflow/state/current.json`은 **`{}`(빈 객체)로 초기화해 포함한다**. 원본 진행 상태를 따라 옮기지 않되 파일 자체는 둔다. 새 작업 디렉토리에서 첫 세션이 시작되면 메인이 (main_full_procedure를 따라) "`{}` = 신규 요청"으로 자연스럽게 진입한다.
...
4. 대상 디렉토리의 `.workflow/state/current.json`을 `{}`(빈 객체) 한 줄로 덮어써 초기화한다 (원본 진행 상태는 따라오지 않게, 파일 자체는 보존).
