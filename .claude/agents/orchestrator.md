...
2. `coder` 서브에이전트 호출. 입력 계약 — 프롬프트에 다음 3 섹션을 순서대로 인용 ([main_full_procedure.md §3.1](../rules/main_full_procedure.md) 참조):
   ...
   전역 금지(`.git/`·`.workflow/state/`·`.claude/agents/`(메모리 외)·lock·CI 등)는 [coder.md](coder.md)에 박혀 있어 매번 인용하지 않는다.
...
4. **history write** (done/aborted/blocked 모든 종료 분기에 적용):
   - `run_id` 생성: `<ISO-timestamp>__<engine>__<slug>` (engine = `claude`/`codex`, slug = 요청 첫 줄 kebab-case 30자 컷, 예: `2026-05-14T10-30-00__claude__add-history-store`)
   ...
   - `.workflow/state/history/index.jsonl`에 한 줄 append:
     ```json
     {"run_id":"<run_id>","engine":"claude","status":"done","mode":"full","request":"<request 한 줄>","files_changed":<N>,"retry_count":<N>,"final_verdict":"Pass"}
     ```
