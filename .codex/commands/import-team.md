...
- **1개**: 그 폴더를 소스로 확정.
- **2개 이상**: 구조화된 사용자 입력 도구 카드로 선택받기 (옵션 라벨에 폴더명 그대로, 가장 최근 mtime이 Recommended).
...
- `.codex/roles/` 전체
- `.codex/rules/` 전체
- `.codex/commands/` 전체 (이 커맨드 자기 자신 `import-team.md`도 포함될 수 있음 — 자기 갱신 허용)
- `.workflow/state/README.md`
...
- **변경 전 승인 필수** — 3단계 구조화된 사용자 입력 도구 승인 없이는 5단계로 진입하지 않는다.
...
3. 3단계에서 미리보기를 사용자에게 보이고 구조화된 사용자 입력 도구 승인 받기. 취소면 즉시 종료.
