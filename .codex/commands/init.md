...
새 프로젝트(=사용자 코드베이스)에 들어왔을 때 Codex가 코드를 읽고 의도를 파악하되, 확신 못 하는 지점은 사용자에게 묻고 그 결과를 `.codex/memory/`(사실)와 `.codex/rules/`(행동 지시)로 분리해 기록하는 커맨드.

한 실행에서 critical 4문항만 묻고 종료. 나머지는 `project-intent.md`의 Unresolved 섹션에 적어두고 다음 `/init` 재실행 때 거기서부터 이어 처리한다.

## 실행 절차

### 0단계 — 재실행 감지

`.codex/memory/project-intent.md`가 존재하는지 확인한다.

- **있음** → 구조화된 사용자 입력 도구 카드로 결정 받기:
  - Unresolved 이어서 (Recommended, Unresolved 섹션에 항목이 있을 때)
  - 처음부터 다시 (스캔·가설부터 새로 — 기존 파일 백업 후 덮어쓰기)
  - 취소
- **없음** → 1단계로.

처음부터 다시를 선택하면 적용 전 백업한다 (`.codex/memory/.backup-<YYYYMMDD_HHMMSS>/project-intent.md`).

### 1단계 — 스캔

다음 정보를 모은다 (Codex 단독 수행, 사용자 노출 X):
...
- 기존 `.codex/memory/project-intent.md`가 있으면 Unresolved 섹션 추출 (재실행 경로용)

### 2단계 — 1차 가설 작성 (내부)

스캔 결과를 토대로 Codex가 독자적으로 정리한다. 이 단계 산출물은 사용자에게 노출하지 않는다 (3단계에서 모호함만 추출해 사용).
...
### 4단계 — 구조화된 사용자 입력 도구 (4문항 한 배치)

한 호출에 최대 4개 질문 카드를 묶어 띄운다. 각 질문에는 Codex의 1차 추측을 (Recommended) 선택지로 포함해 사용자가 빠르게 확정할 수 있게 한다.

각 카드의 옵션 구성:
- Codex의 1차 추측 (Recommended)
- 대안 추측 (있으면 1~2개)
...
- `.codex/memory/project-intent.md`의 Unresolved 섹션에서 다음 4개 항목을 추출한다.
- 동일한 구조화된 사용자 입력 도구 패턴으로 묻는다.
- 응답을 받으면 해당 항목을 Unresolved에서 제거하고 본문에 반영한다.

### 5단계 — 기록 (memory/rules 분리)

#### `.codex/memory/project-intent.md` 작성/갱신
...
#### `.codex/rules/<주제>.md` 작성 (행동 지시가 도출됐을 때만)
...
- 형식은 글로벌 `~/.codex/rules/` 패턴을 따른다 (짧은 본문, **Why:** / **How to apply:** 라인 권장)

#### 인덱스 처리

- `.codex/memory/MEMORY.md` 존재 → `project-intent.md` 항목 한 줄 추가/갱신.
- `.codex/rules/RULES.md` 존재 → 새로 만든 rules 파일 항목 한 줄 추가.
- **인덱스 파일이 없으면** 구조화된 사용자 입력 도구 카드로 결정 받기:
...
- `.codex/memory/` 없으면 생성.
- `.codex/rules/` 없으면 생성.
- 생성한 폴더가 있으면 채팅에 한 줄 알림 (예: "신규 폴더 생성: `.codex/memory/`, `.codex/rules/`").
...
산출물: .codex/memory/project-intent.md
프로젝트: <한 줄 정의>
이번 회차 해소: N개
Unresolved 남음: M개

다음 /init 재실행 시 Unresolved부터 이어 처리합니다.
```
...
- `.git/`, `.workflow/state/`, lock 파일, CI 설정은 변경하지 않는다.
- 본 커맨드는 **오직** `.codex/memory/`, `.codex/rules/` 하위와 그 인덱스(`MEMORY.md`, `RULES.md`)만 생성/수정한다.
- 기존 `project-intent.md`를 덮어쓰는 경로(0단계의 "처음부터 다시")에서는 백업 디렉토리(`.codex/memory/.backup-<timestamp>/`)에 원본을 먼저 복사한다.

## 설계 원칙 (이 커맨드가 따르는 것)

- **묻기 전에 추측한다** — 무작정 질문 폭격이 아니라 1차 가설 작성 후 모호한 지점만 추림.
- **한 번에 critical 4개** — 사용자 피로 최소화. 나머지는 Unresolved로 이연.
- **사실은 memory, 행동은 rules** — 글로벌 분류 체계 (`~/.codex/`)와 동일.
