...
이 프로젝트의 **full 모드 비단순 요청**은 아래 4단계 사이클을 따른다. 메인 Codex는 `/full` prefix가 붙은 요청을 받으면 [`main_full_procedure.md`](main_full_procedure.md)를 따라 4단계 진행을 직접 지휘한다. 단계별 상세 규약은 [`../roles/orchestrator.md`](../roles/orchestrator.md)에 보존된다. 단계 전이마다 `.workflow/state/current.json`을 갱신해 세션이 끊겨도 재개 가능하게 한다.

**lite 모드**는 본 4단계를 적용하지 않는다 — 메인이 직접 파일 편집으로 외과적 수정 후 critic만 호출. 자세한 흐름은 본 문서 하단 "Lite 모드 (기본)" 절 참조.

## 모드 분기 (Lite vs Full)

| 모드 | 진입 조건 | 실행 |
|------|----------|------|
| **lite** (기본) | `/full` prefix 없음 | 메인이 직접 파일 편집 → critic 검증 (2단계: 수정→검증, 역할 1개) |
| **full** | `/full` prefix 있음 | 메인이 4단계 사이클 지휘 (search→plan→implement→verify) + report (역할 4개) |
...
**state.mode 필드**: full 모드에서만 current.json을 초기화하고 `"mode": "full"` 기록. lite는 current.json을 생성·갱신하지 않는다 (재개 불필요).
...
1. 메인이 사용자 요청을 한 줄로 정리하고 `current.json` 초기화 (engine=`codex`, status=`searching`, current_phase=`search`). 같은 시점에 `temp/progress.md`를 run 헤더로 덮어써 초기화한다.
...
4. 메인이 **구조화된 사용자 입력 도구** 카드로 결정 받기 — 옵션 예:
...
3. planner가 다음을 반환:
   ...
   - 영향 범위·예상 리스크
   - **implementer 변경 명세** (대상·금지 사항·완료 조건·코드 컨벤션)
   - **사용자 결정 항목** (구조화된 사용자 입력 도구 카드용 옵션)
4. planner 결과 본문을 `temp/plan.md`에 덮어쓰고 `temp/progress.md`에 기획 산출 경로를 append한다. 채팅에는 기획서 링크와 핵심 한 줄만 남기고, 본문 전체를 붙이지 않는다.

출력 위치: planner 산출물 본문은 `temp/plan.md`에 덮어쓰기로 작성. 채팅에는 `temp/plan.md` 링크와 구조화된 사용자 입력 도구 승인 카드만 띄운다.

5. 메인이 **구조화된 사용자 입력 도구** 카드로 결정 + 승인 — 한 호출에 최대 4개 질문 묶음, 마지막은 항상 "승인 / 수정요청 / 취소".
6. 응답 처리:
...
2. implementer 호출 — 프롬프트에 세 섹션을 순서대로 인용한다 (자세한 형식: [main_full_procedure.md §3.1](main_full_procedure.md)):
   - `## 사용자 원 요청` — 첫 메시지 본문 그대로 (인텐트 lossy 방지)
   - `## planner 변경 명세` — planner 산출 블록 그대로
   - `## (재호출 시) 이전 시도 실패 사유` — critic 사유 목록 (Phase 4 Fail 후 재구현 때만)
3. implementer가 변경 명세에 적힌 파일·심볼만 파일 편집으로 직접 변경한다. 변경 명세에 검증 명령(테스트·빌드)이 있으면 shell로 실행해 자체 점검.
4. implementer가 결과(변경된 파일 목록·신규/삭제 파일·자체 검증 결과·행동 4원칙 자기 점검)를 반환.
5. 메인이 결과 요지를 기록하고 `temp/progress.md`에 구현 시도 요약을 append한다.
6. implementer가 실패(Edit 도구 오류, 가정 충돌, 자체 검증 실패 등)를 보고했으면 Phase 4의 실패 경로로 직행.

**핵심 강제 메커니즘**: 파일 편집는 **implementer에만** 부여된다. researcher/planner/critic은 도구 목록 자체에 파일 편집가 없어 구조적으로 코드를 편집할 수 없다. full 모드의 메인은 단계 전이 메타데이터(`.workflow/state/**`, `temp/**` 등) 화이트리스트만 쓰고 코드 파일은 손대지 않는다. lite 모드의 메인은 본 메커니즘과 별개로, 자기 파일 편집으로 코드를 직접 외과적 수정한다 (AGENTS.md "Lite 모드" 절 참조).
...
2. critic 호출 — 입력: 사용자 원 요청, planner 완료 조건·금지 사항, implementer 변경 보고, 테스트 명령 (있으면).
...
- **retry_count < 3** (1회·2회 실패):
  - 메인이 **구조화된 사용자 입력 도구** 카드 — 옵션:
    - **자동 재구현** (Recommended) — critic 사유를 implementer 입력에 추가해 Phase 3 재실행
...
#### 재시도 시 implementer 프롬프트 보강
critic 사유를 implementer 호출 프롬프트 말미에 추가:
```text
...
출력 위치: 완료 보고 본문은 `temp/report.md`에 덮어쓰기로 작성. 채팅에는 `temp/report.md` 링크와 한 줄 요약만. 영속본은 별도 파일이 아니라 `.workflow/state/history/<run_id>/run.json`의 `report_body` 필드에 흡수한다 (도구 가드가 `report.md` Write 차단 → run당 단일 파일).
...
**모든 세션 시작 시 가장 먼저 `.workflow/state/current.json` Read**. (AGENTS.md 진입점)

- 파일이 없거나 status가 `done`/`aborted` → 신규 요청.
- status가 `searching` / `planning` / `awaiting_approval` / `approved` / `implementing` / `verifying` → 구조화된 사용자 입력 도구로 "재개 / 처음부터 재계획 / 취소".
...
- **current.json이 SoT**. 워크플로우의 모든 진행 상태·재시도 카운터는 current.json을 기준으로 판단.
- 작업 목록는 사용자 가시성을 위한 **보조 view**. current.json에서 derive돼 표시되는 것으로 취급.
- 갱신 순서: **current.json 먼저 → 작업 목록 동기화**. 역순 금지.
- 둘 사이 불일치가 발견되면 current.json을 신뢰, 작업 목록 재구성.

### 갱신 규칙
- state 갱신은 매 단계 전이마다 한다 (배치로 미루지 않음). 세션이 어디서 끊겨도 손실을 한 단계 이내로 제한.
- current.json 쓰기는 메인만 수행. researcher/planner/implementer/critic은 결과만 반환.

### history 영속화

모든 워크플로우 실행의 종료(done/aborted/blocked) 기록을 `.workflow/state/history/`에 영속화한다.
...
**`run_id` 형식**: `<ISO-timestamp>__codex__<slug>`
- ISO-timestamp: `YYYY-MM-DDTHH-MM-SS` (콜론 대신 하이픈, UTC+9)
- engine: 항상 `codex`
- slug: 요청 첫 줄 kebab-case 30자 컷
- 예: `2026-05-14T10-30-00__codex__add-history-store`

**run.json 스키마**:
```json
{
  "run_id": "2026-05-14T10-30-00__codex__add-history-store",
  "engine": "codex",
...
**index.jsonl 한 줄 스키마**:
```json
{"run_id":"2026-05-14T10-30-00__codex__add-history-store","engine":"codex","status":"done","mode":"full","request":"변경 이력 저장 구조 추가","files_changed":2,"retry_count":0,"final_verdict":"Pass"}
```

**쓰기 책임**:
- **메인만** history에 쓴다. researcher/planner/implementer/critic은 직접 쓰지 않는다.
...
2. [AGENTS.md](../../AGENTS.md) — "변경 시 원칙" 섹션
3. [orchestrator.md](../roles/orchestrator.md) — "Phase 4" 섹션
4. [main_full_procedure.md](main_full_procedure.md) — §2 4단계 순서 흐름도

**lite 자동 재시도 횟수(1회)**를 바꾸려면 본 파일 "Lite 모드 실행 흐름" §5 + [AGENTS.md](../../AGENTS.md) "Lite 모드" 절을 함께 갱신.

## 출력 위치 규약

사용자가 `input/<파일명>` 식으로 언급하면 Codex는 `temp/input/<파일명>`을 참조 대상으로 해석한다. 사용자가 보고서/문서로 만들어줘 식으로 산출물을 요청하면 Codex는 `temp/output/`에 작성하고 경로 링크로 회신한다.

## 구조화된 사용자 입력 도구 사용 지점 (요약)

메인은 다음 모든 분기에서 구조화된 사용자 입력 도구 카드를 띄운다:
...
Lite 모드는 4단계 사이클을 적용하지 않는다. 메인 Codex가 직접 외과적으로 파일 편집를 수행하고 critic 역할만 호출해 결과를 검증한다. researcher·planner·implementer는 lite에서 호출되지 않는다 (orchestrator.md는 비활성 참조 절차서이므로 lite·full 모두에서 호출되지 않음).

### Lite 모드 실행 흐름

1. **상태 확인**: 메인이 `.workflow/state/current.json`을 읽어 in-progress·blocked 상태가 있으면 재개 여부 질의 후 그쪽으로 처리(이 경우 full 워크플로우일 가능성 높음 — 메인이 main_full_procedure 4단계로 진입). 없으면 lite 신규 처리.
2. **외과적 수정**: 메인이 파일 편집으로 사용자 요청을 직접 처리. 변경 줄이 사용자 요청에 직접 트레이스되도록 ([coding_principles.md](coding_principles.md) §3).
...
4. **판정 처리**:
   ...
   - **Fail**: 메인이 critic 사유를 받아 **자동 재수정 1회** 후 critic 재호출.
     - **재호출 Pass**: 종료.
     - **재호출 Fail**: 구조화된 사용자 입력 도구(자동 재구현 한번 더 / `/full`로 승격 / 중단). 응답 처리:
       - "자동 재구현 한번 더" → 메인이 critic 누적 사유로 한 번 더 시도 → critic 재호출 → Pass면 종료, 또 Fail이면 사용자에게 보고하고 종료(이상 자동 재시도 없음).
       - "/full로 승격" → current.json에 mode=full로 초기화하고 메인이 main_full_procedure 4단계 진입 (Phase 1부터 재시작).
       - "중단" → 메인이 history write 후 종료.
5. **history write**: 메인이 종료(Pass·아니면 중단) 시점에 `.workflow/state/history/<run_id>/run.json`을 직접 작성. 스키마는 full과 동일. `final_verdict` = `Pass|Fail|N/A`. 작성 후 별도 current.json 초기화는 불필요(lite는 current.json을 안 만들었으므로).

### Lite vs Full 동작 차이

| 항목 | lite | full |
|------|------|------|
| 진입점 | 메인이 직접 파일 편집 | 메인이 main_full_procedure 4단계 진행 |
| 호출되는 역할 | critic만 | researcher → planner → implementer → critic (메인이 지휘) |
| 단계 | 수정 → 검증 (단계 표현 없음) | Phase 1(search) → Phase 2(plan, 승인 필수) → Phase 3(implement) → Phase 4(verify) |
| 변경 주체 | 메인 자체 파일 편집 | implementer 역할 파일 편집 |
| critic 입력 | 사용자 원문 + 변경 파일 + 행동4원칙 | planner 완료조건 + 금지사항 + implementer 변경 보고 |
...
| current.json | **생성 안 함** | 매 단계 전이마다 갱신 |
