#include "LD06.h"
#include <fcntl.h>
#include <unistd.h>
#include <termios.h>
#include <mutex>
#include <shared_mutex>

namespace LD06
{
	using namespace std;

	static const uchar CrcTable[256] = {
		};

	bool Lidar::Init()
	{
		m_fd = -1;
		m_fd = open("/dev/ttyUSB0", O_RDWR | O_NOCTTY | O_NONBLOCK);
		if (m_fd < 0)
		{
			printf("Fail to open serial protocol for LD06\n");
			return false;
		}

		struct termios tty = {};
		if (tcgetattr(m_fd, &tty) != 0)
		{
			printf("Fail to tcgetattr serial protocol for LD06\n");
			close(m_fd);
			return false;
		}

		cfsetospeed(&tty, B230400);
		cfsetispeed(&tty, B230400);

		tty.c_cflag |= (CLOCAL | CREAD); // enable receiver, local mode
		tty.c_cflag &= ~CSIZE;
		tty.c_cflag |= CS8;								// 8-bit
		tty.c_cflag &= ~PARENB;							// no parity
		tty.c_cflag &= ~CSTOPB;							// 1 stop bit
		tty.c_cflag &= ~CRTSCTS;						// no flow control
		tty.c_lflag &= ~(ICANON | ECHO | ECHOE | ISIG); // raw input
		tty.c_iflag &= ~(IXON | IXOFF | IXANY);			// no software flow control
		tty.c_oflag &= ~OPOST;							// raw output

		tcflush(m_fd, TCIFLUSH);
		if (tcsetattr(m_fd, TCSANOW, &tty) != 0)
		{
			printf("Fail to tcsetattr serial protocol for LD06\n");
			close(m_fd);
			return false;
		}

		m_isStop = false;
		m_recvThread = thread(&Lidar::reciveSerialDataThreadFunc, this);

		return true;
	}
	void Lidar::Release()
	{
		if (m_fd < 0)
			return;

		m_isStop = true;
		m_recvThread.join();
		close(m_fd);
		m_fd = -1;
	}

	bool Lidar::isValidData(int *data, int len)
	{
		uchar crc = 0;
		for (int i = 0; i + 1 < len; i++)
			crc = CrcTable[(uchar)((crc ^ data[i]) & 0xff)];

		return crc == data[len - 1];
	}
	int Lidar::concatBytes(int &left, int &right)
	{
		return left | right << 8;
	}
	void Lidar::reciveSerialDataThreadFunc()
	{
		pthread_setname_np(pthread_self(), "Lidar Data Generator Thread");

		bool headerCheck = false;
		bool verlenCheck = false;
		int bufIdx = 2;
		vector<int> rawBuf(47);
		vector<LidarData> packetBuf(12);
		vector<LidarData> localData;
		float startDegree;
		float endDegree;
		float diffDegree;
		rawBuf[0] = 0x54;
		rawBuf[1] = 0x2C;

		uint8_t byte = 0;

		while (!m_isStop)
		{
			int n = read(m_fd, &byte, 1);
			if (n < 0)
			{
				if (errno != EAGAIN && errno != EWOULDBLOCK)
					perror("Serial read error");
				this_thread::sleep_for(1ms);
				continue;
			}
			else if (n == 0)
			{
				this_thread::sleep_for(1ms);
				continue;
			}

			int val = byte;

			if (val == 0x54 && !headerCheck && !verlenCheck)
			{
				headerCheck = true;
				verlenCheck = false;
			}
			else if (val == 0x2C && headerCheck && !verlenCheck)
			{
				verlenCheck = true;
			}
			else if (headerCheck && verlenCheck)
			{
				rawBuf[bufIdx++] = val;
				if (bufIdx < 47)
					continue;

				bufIdx = 2;
				headerCheck = false;
				verlenCheck = false;

				if (!isValidData(rawBuf.data(), 47))
					continue;

				startDegree = concatBytes(rawBuf[4], rawBuf[5]) * 0.01f;
				endDegree = concatBytes(rawBuf[42], rawBuf[43]) * 0.01f;
				diffDegree = endDegree - startDegree;
				if (diffDegree < 0)
					diffDegree += 360.f;
				diffDegree /= 11.f;

				for (int i = 0; i < 12; i++)
				{
					float curDeg = (diffDegree * i + startDegree);
					if (curDeg > 360.f)
						curDeg -= 360.0;

					packetBuf[i].Degree = curDeg;
					packetBuf[i].Distance = concatBytes(rawBuf[3 * i + 6], rawBuf[3 * i + 7]);
					packetBuf[i].Intensity = rawBuf[3 * i + 8];
				}

				for (int i = 0; i < 12; i++)
				{
					LidarData t_data;
					t_data.Degree = packetBuf[i].Degree;
					t_data.Distance = packetBuf[i].Distance;
					t_data.Intensity = packetBuf[i].Intensity;
					localData.push_back(t_data);

					if (i < 11 && packetBuf[i].Degree > packetBuf[i + 1].Degree)
					{
						{
							unique_lock lock(m_dataMutex);
							m_data = localData;
						}
						localData.clear();
					}
				}
			}
		}
	}

	std::vector<LidarData> Lidar::GetData()
	{
		vector<LidarData> ret;
		{
			shared_lock lock(m_dataMutex);
			ret = m_data;
		}
		return ret;
	}
}
